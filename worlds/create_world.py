...
def create_no_roof_no_walls_small_warehouse_9_corner():
    ...
    for element in root.iter():
        if element.tag in ["model", "light"]:
            if element.tag not in ["gravity", "physics", "gui"] and element.attrib["name"] in [
                ...
                for x_count, x_val in enumerate([0, GROUND_X, 2 * (GROUND_X)]):
                    for y_count, y_val in enumerate(
                        [0, GROUND_Y + SPACE_CORRIDORS, 2 * (GROUND_Y + SPACE_CORRIDORS)]
                    ):
    ...
    for element in root.iter():
        if element.tag in ["model", "light"]:
            if (
                ...
                and element.attrib["name"] == "aws_robomaker_warehouse_GroundB_01_001"
            ):
                for x_count, x_val in enumerate([0, -GROUND_X, GROUND_X, 2 * GROUND_X, 3 * GROUND_X]):
                    for y_count, y_val in enumerate(
                        [0, -GROUND_Y, GROUND_Y, 2 * GROUND_Y, 3* GROUND_Y]
                    ):
                        if x_count + y_count != 0:
                            new_model = copy.deepcopy(element)
                            new_model.set("name", f"{new_model.attrib['name']}_{x_count}_{y_count}")
    ...
    with open("no_roof_no_walls_small_warehouse_9_corner.world", "w") as out:
        out.write(xml)
        ...
        if element.tag in ["model", "light"]:
            if (
                ...
                for x_count, x_val in enumerate(
                    [0, GROUND_X + SPACE_CORRIDORS, -GROUND_X - SPACE_CORRIDORS]
                ):
        ...
        if element.tag in ["model", "light"]:
            if (
                ...
                for x_count, x_val in enumerate(
                    [
                        0,
                        GROUND_X + SPACE_CORRIDORS,
                        -GROUND_X - SPACE_CORRIDORS,
                        2 * (GROUND_X + SPACE_CORRIDORS),
                        -2 * (GROUND_X + SPACE_CORRIDORS),
                    ]
                ):
                    for y_count, y_val in enumerate(
    ...
    for element in root.iter():
        if element.tag in ["model", "light"]:
            if element.tag not in ["gravity", "physics", "gui"] and element.attrib["name"] in [
                ...
                for x_count, x_val in enumerate([0, GROUND_X + SPACE_CORRIDORS]):
                    for y_count, y_val in enumerate([0, GROUND_Y, -GROUND_Y]):
                        if x_count + y_count != 0:
                            new_model = copy.deepcopy(element)
                            new_model.set("name", f"{new_model.attrib['name']}_{x_count}_{y_count}")
...
if __name__ == "__main__":
    ...
    elif args.world_name == "no_roof_no_walls_small_warehouse_9_corner":
        create_no_roof_no_walls_small_warehouse_9_corner()
